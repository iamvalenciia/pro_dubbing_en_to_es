...
class TransCreate(BaseTask):
    ...
    def _tts(self, daz_json=None) -> None:
        ...
        use_speaker_clone_refs = force_clone and bool(speaker_marks)
        ...
        for i, it in enumerate(subs):
            ...
            # 确保voice总是字符串，处理None或其他类型的值（edge-tts要求voice必须是字符串）
            voice = str(voice).strip() if voice else str(voice_role).strip() if voice_role else 'en-US-AriaNeural'
        ...
        if len([it.get("ref_wav") for it in self.queue_tts if it.get("ref_wav")]) > 0:
            ...
            for it in self.queue_tts:
                ref_wav = it.get('ref_wav')
                if ref_wav and hasattr(self, 'ref_wav_durations'):
                    total_duration = self.ref_wav_durations.get(ref_wav, 0)
                    it['with_emotion'] = total_duration >= 60_000  # True if >= 60s
                else:
                    it['with_emotion'] = False
    ...
    def _create_ref_from_vocal(self):
        ...
        # Build unique ref clips. For speaker-aware mode, collect ALL segments per
        # speaker and concatenate (up to MAX_REF_MS ms = 60s) so the x-vector extractor
        ...
        # Sort each group longest-first so we fill the budget with the best segments.
        for ref_wav in ref_segment_groups:
            ref_segment_groups[ref_wav].sort(key=lambda x: x[0], reverse=True)
        ...
        def _build_ref_for_speaker(ref_wav, seg_list):
            try:
                ...
                if len(tmp_clips) == 1:
                    import shutil
                    shutil.move(tmp_clips[0], ref_wav)
                else:
                    # Concatenate clips via ffmpeg concat demuxer
                    concat_txt = ref_wav + '.concat.txt'
                    ...
                    for cp in tmp_clips:
                        try:
                            ...
                            pass
                    try:
                        _os.remove(concat_txt)
                    except Exception:
                        pass
                
                # Store the total duration for this speaker reference
                self.ref_wav_durations[ref_wav] = total_ms
                logger.debug(f"Reference built: {ref_wav} from {len(tmp_clips)} clip(s), {total_ms}ms total")
            except Exception as e:
