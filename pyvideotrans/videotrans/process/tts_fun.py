...
def qwen3tts_fun(
    ...
    try:
        ...
        for i,it in enumerate(queue_tts):
            ...
            # Adjust temperature based on reference duration:
            # - with_emotion=False (short <60s ref): temp=0.7 for clear, natural voice without emotional variations
            # - with_emotion=True (long >=60s ref):  temp=0.9 for more expressive, emotional delivery
            with_emotion = it.get('with_emotion', False)
            if with_emotion:
                kw['temperature'] = 0.9  # More expressive with emotion
                kw['top_p'] = 0.92
            else:
                kw['temperature'] = 0.7  # Clearer, more neutral without emotion
                kw['top_p'] = 0.85
            
            if not ref_text:
                # Cross-lingual clone: x_vector_only strips phonetics from reference
                # embedding so the model synthesises native Spanish instead of accented.
